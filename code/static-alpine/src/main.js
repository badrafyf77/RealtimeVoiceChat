...
document.getElementById('app').innerHTML = `
  <div x-data="voiceChat" class="flex flex-col h-screen bg-zinc-50 dark:bg-zinc-900">
    ...
    <main class="flex-1 flex h-full">
      ...
      <div class="flex-1 flex items-center justify-center p-5 sm:p-8 bg-zinc-50 dark:bg-zinc-900">
      ...
      <!-- Conversation Panel -->
      <div class="w-full lg:w-96 xl:w-[28rem] bg-white dark:bg-zinc-800 border-l border-zinc-200 dark:border-zinc-700 flex flex-col h-full">
        <!-- Header -->
        <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 flex-shrink-0">
          <h2 class="text-lg font-semibold text-zinc-900 dark:text-white">Conversation</h2>
          <p class="text-sm text-zinc-600 dark:text-zinc-400">Real-time transcript</p>
        </div>
        
        <!-- Messages Container -->
        <div class="flex-1 relative overflow-hidden">
          <!-- Messages List -->
          <div x-ref="transcript" class="h-full overflow-y-auto px-6 py-4 space-y-4 scrollbar-hidden">
          ...
          <div x-show="userScrolled && messages.length > 0" 
               ...
               class="absolute bottom-4 right-4 bg-blue-600 hover:bg-blue-700 text-white p-2 rounded-full shadow-lg cursor-pointer transition-all duration-200">
